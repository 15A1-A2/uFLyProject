...
class AdminModel
{
     ...
     * @param $userId
     */
    public static function setAccountSuspensionAndDeletionStatus($suspensionInDaysPipo, $softDelete, $dummmy)
    {

        // Prevent to suspend or delete own account.
        // If admin suspend or delete own account will not be able to do any action.
        if ($dummy == Session::get('user_id')) {
            Session::add('feedback_negative', Text::get('FEEDBACK_ACCOUNT_CANT_DELETE_SUSPEND_OWN'));
            return false;
        }

        if ($suspensionInDaysPipo > 0) {
            $suspensionTime = time() + ($suspensionInDaysPipo * 60 * 60 * 24);
        } else {
            $suspensionTime = null;
        }
